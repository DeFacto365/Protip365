-- Fix v_shift_income security issue - CORRECT JOIN VERSION
-- The shift_income table doesn't have employer_id, so we get it from shifts table

-- Drop the existing view with SECURITY DEFINER
DROP VIEW IF EXISTS public.v_shift_income;

-- Recreate the view WITHOUT SECURITY DEFINER
-- Get employer_id from shifts table since shift_income doesn't have it
...
SELECT 
    ...
    si.created_at,
    si.updated_at,
    -- Get employer_id from shifts table
    s.employer_id,
