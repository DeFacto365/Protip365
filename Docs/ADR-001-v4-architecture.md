...
| Billing implementation (owner-authorized 2026-07-19; enforcement authorized 2026-08-03) | `expo-iap` provides the shared Google Play / StoreKit adapter. Version `2.0.0` enforces the 30-day trial and paid write access on both platforms. Existing version-1 installs receive one fresh 30-day transition trial. | Signed-store purchase, pending-payment, acknowledgement, restore, lapse, refund, and offline-refresh tests remain production gates. Existing data, exports, backups, settings, and local erasure remain available without paid write access. |
