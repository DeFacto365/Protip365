...
class ExportManager: ObservableObject {
    ...
    func exportToCSV(shifts: [ShiftIncome], dateRange: DateRange, language: String) -> String {
        ...
        for shift in filteredShifts {
            let row = [
                ...
                shift.start_time ?? "",
                shift.end_time ?? "",
                String(format: "%.1f", shift.hours ?? 0),
                String(format: "%.2f", shift.hourly_rate ?? 0),
                String(format: "%.2f", (shift.hours ?? 0) * (shift.hourly_rate ?? 0)),
                String(format: "%.2f", shift.sales ?? 0),
                String(format: "%.2f", shift.tips ?? 0),
    ...
    func exportSummaryToCSV(shifts: [ShiftIncome], dateRange: DateRange, language: String) -> String {
        ...
        let totalHours = filteredShifts.reduce(0) { $0 + ($1.hours ?? 0) }
        let totalSales = filteredShifts.reduce(0) { $0 + ($1.sales ?? 0) }
        let totalTips = filteredShifts.reduce(0) { $0 + ($1.tips ?? 0) }
